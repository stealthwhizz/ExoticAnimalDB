-- PostgreSQL version of ExoticAnimalDB
-- Drop and create database (run as postgres superuser)
-- DROP DATABASE IF EXISTS exoticanimaldb;
-- CREATE DATABASE exoticanimaldb;
-- \c exoticanimaldb;

-- Drop existing tables if they exist
DROP TABLE IF EXISTS BreedingRecords CASCADE;
DROP TABLE IF EXISTS MedicalRecords CASCADE;
DROP TABLE IF EXISTS AnimalCare CASCADE;
...
DROP VIEW IF EXISTS ActiveAnimalsView CASCADE;
DROP VIEW IF EXISTS KeeperAssignmentsView CASCADE;

-- Drop functions if they exist
DROP FUNCTION IF EXISTS getAnimalAgeYears(INT);
DROP FUNCTION IF EXISTS totalMedicalCost(INT);
DROP FUNCTION IF EXISTS healthRiskScore(INT);

-- Drop procedures if they exist
DROP PROCEDURE IF EXISTS addAnimal(VARCHAR, INT, INT, VARCHAR, DATE, DATE, VARCHAR, VARCHAR, DECIMAL, VARCHAR);
DROP PROCEDURE IF EXISTS assignPrimaryKeeper(INT, INT, TEXT);
DROP PROCEDURE IF EXISTS recordMedicalVisit(INT, VARCHAR, DECIMAL, TEXT, TEXT, VARCHAR, DATE, VARCHAR);

-- Create custom types for ENUMs
CREATE TYPE conservation_status_type AS ENUM ('Least Concern', 'Near Threatened', 'Vulnerable', 'Endangered', 'Critically Endangered', 'Extinct');
CREATE TYPE diet_type AS ENUM ('Carnivore', 'Herbivore', 'Omnivore', 'Insectivore');
CREATE TYPE facility_type AS ENUM ('Zoo', 'Wildlife Sanctuary', 'Aquarium', 'Research Center');
CREATE TYPE gender_type AS ENUM ('Male', 'Female', 'Unknown');
CREATE TYPE acquisition_method_type AS ENUM ('Born in facility', 'Purchased', 'Rescued', 'Transferred', 'Donated');
CREATE TYPE health_status_type AS ENUM ('Excellent', 'Good', 'Fair', 'Poor', 'Critical');
CREATE TYPE certification_level_type AS ENUM ('Beginner', 'Intermediate', 'Advanced', 'Expert');
CREATE TYPE care_type AS ENUM ('Primary', 'Secondary', 'Temporary', 'Emergency');
CREATE TYPE record_type AS ENUM ('Routine Checkup', 'Emergency', 'Surgery', 'Vaccination', 'Injury');
...
CREATE TABLE Species (
    ...
    conservation_status conservation_status_type DEFAULT 'Least Concern',
...
CREATE TABLE Keepers (
    ...
    certification_level certification_level_type DEFAULT 'Beginner',
...
CREATE TABLE Animals (
    ...
    acquisition_method acquisition_method_type NOT NULL,
    health_status health_status_type DEFAULT 'Good',
...
CREATE TABLE AnimalCare (
    ...
    care_type care_type DEFAULT 'Primary',
    notes TEXT,
...
CREATE INDEX idx_animal_keeper ON AnimalCare(animal_id, keeper_id);
...
CREATE OR REPLACE FUNCTION getAnimalAgeYears(p_animal_id INT)
RETURNS INT AS $$
DECLARE
  bdate DATE;
  yrs INT;
BEGIN
  ...
  IF bdate IS NULL THEN
    yrs := 0;
  ELSE
    yrs := EXTRACT(YEAR FROM AGE(CURRENT_DATE, bdate));
  END IF;
  RETURN yrs;
END;
$$ LANGUAGE plpgsql;

CREATE OR REPLACE FUNCTION totalMedicalCost(p_animal_id INT)
...
CREATE OR REPLACE FUNCTION healthRiskScore(p_animal_id INT)
RETURNS INT AS $$
DECLARE
  status VARCHAR(50);
  base INT := 0;
  recent_emergencies INT := 0;
BEGIN
  SELECT s.conservation_status::TEXT INTO status
  FROM Animals a
  JOIN Species s ON a.species_id = s.species_id
  WHERE a.animal_id = p_animal_id;

  IF status IN ('Critically Endangered') THEN base := 5;
  ELSIF status IN ('Endangered') THEN base := 4;
  ELSIF status IN ('Vulnerable') THEN base := 3;
  ELSIF status IN ('Near Threatened') THEN base := 2;
  ELSE base := 1;
  END IF;
  ...
  WHERE animal_id = p_animal_id
    ...
    AND visit_date >= CURRENT_DATE - INTERVAL '180 days';
...
CREATE OR REPLACE PROCEDURE addAnimal(
  p_animalname VARCHAR(100),
  p_speciesid INT,
  p_facilityid INT,
  p_gender gender_type,
  p_birthdate DATE,
  p_acquisitiondate DATE,
  p_acquisitionmethod acquisition_method_type,
  p_healthstatus health_status_type,
  p_weightkg DECIMAL(8,2),
  p_microchipid VARCHAR(20)
)
LANGUAGE plpgsql AS $$
DECLARE
  cap INT;
...
BEGIN
  ...
  IF current_count >= cap THEN
    RAISE EXCEPTION 'Facility capacity exceeded';
  END IF;
...
CREATE OR REPLACE PROCEDURE assignPrimaryKeeper(
  p_animal_id INT,
  p_keeper_id INT,
  p_notes TEXT
)
...
BEGIN
  IF (SELECT COUNT(*) FROM Animals WHERE animal_id = p_animal_id AND is_active = TRUE) = 0 THEN
    RAISE EXCEPTION 'Invalid or inactive animal';
  END IF;
  IF (SELECT COUNT(*) FROM Keepers WHERE keeper_id = p_keeper_id) = 0 THEN
    RAISE EXCEPTION 'Invalid keeper';
  END IF;
  ...
  VALUES (p_animal_id, p_keeper_id, CURRENT_DATE, 'Primary', p_notes, TRUE);
END;
$$;

CREATE OR REPLACE PROCEDURE recordMedicalVisit(
  p_animal_id INT,
  p_recordtype record_type,
  p_cost DECIMAL(10,2),
  p_diagnosis TEXT,
  p_treatment TEXT,
  p_medications VARCHAR(500),
  p_followupdate DATE,
  p_veterinarianname VARCHAR(100)
)
LANGUAGE plpgsql AS $$
BEGIN
  ...
  END IF;
  IF p_cost < 0 THEN
    RAISE EXCEPTION 'Negative cost not allowed';
  END IF;
  ...
  INSERT INTO MedicalRecords(
    ...
    p_animal_id, CURRENT_DATE, p_veterinarianname, p_diagnosis, p_treatment, p_medications,
  ...
  IF p_recordtype IN ('Emergency', 'Surgery') THEN
    ...
    WHERE animal_id = p_animal_id AND health_status IN ('Good', 'Fair', 'Excellent');
  END IF;
END;
$$;
...
-- Enforce weight bounds on insert
CREATE OR REPLACE FUNCTION trg_animals_weight_check_fn()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.weight_kg IS NOT NULL AND (NEW.weight_kg < 0.001 OR NEW.weight_kg > 10000) THEN
    RAISE EXCEPTION 'Weight out of allowed bounds';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
FOR EACH ROW
EXECUTE FUNCTION trg_animals_weight_check_fn();

-- Guard facility capacity
CREATE OR REPLACE FUNCTION trg_facility_capacity_fn()
RETURNS TRIGGER AS $$
DECLARE
  cap INT;
  cnt INT;
BEGIN
  ...
  IF cnt >= cap THEN
    RAISE EXCEPTION 'Facility capacity exceeded (trigger)';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
FOR EACH ROW
EXECUTE FUNCTION trg_facility_capacity_fn();

-- Only one current Primary keeper per animal
CREATE OR REPLACE FUNCTION trg_animalcare_primary_uniqueness_fn()
RETURNS TRIGGER AS $$
BEGIN
  IF NEW.is_current = TRUE AND NEW.care_type = 'Primary' THEN
    IF EXISTS (
      ...
      WHERE animal_id = NEW.animal_id AND is_current = TRUE AND care_type = 'Primary'
        AND (TG_OP = 'INSERT' OR (TG_OP = 'UPDATE' AND care_id <> NEW.care_id))
    ) THEN
      RAISE EXCEPTION 'Primary current keeper already exists for this animal';
    END IF;
  END IF;
  RETURN NEW;
END;
...
BEFORE INSERT OR UPDATE ON AnimalCare
FOR EACH ROW
EXECUTE FUNCTION trg_animalcare_primary_uniqueness_fn();
